...
(function () {
  ...
  const TABS = [
    ...
    { id: 'hubspot',    section: 'Strategy',    label: 'HubSpot',        icon: I.target,   title: 'HubSpot · Out-of-Date Deals', sub: 'Aged-deal counts per team · pulled from HubSpot daily' },
  ...
  function render() {
    ...
    else if (tab === 'hubspot')  { host.innerHTML = renderHubspot(); wireHubspot(); }
  ...
  // ─── HubSpot · Out-of-Date Deals ──────────────────────────────────
  // Reads data/hubspot_outdated.json (refreshed daily by the GH Action
  // .github/workflows/fetch-hubspot.yml). Three groups (1/2/3) — each a
  // batch of teams pulled from the RAW DATA DEALS source spreadsheet.
  ...
  function renderHubspot() {
    ...
    // --- last refresh: badge + relative phrasing -----------------------------
    ...
    const ageClass = (() => {
      ...
      if (hrs < 30) return 'ok';      // daily action ran in the last day-ish
    ...
    // KPI helper — same shape as Overview/Live Floor, no delta arrow since
    // there's no week-on-week baseline for this snapshot yet.
    ...
    // --- cell formatting & colour grading -----------------------------------
    ...
    const pctCell = (frac) => {
      ...
      return `<td class="num"><span class="pill ${cls}">${(Number(frac) * 100).toFixed(1)}%</span></td>`;
    };
    ...
    // Sticky-column styling lives on every first-cell so the team name pins
    // while the table scrolls horizontally inside .tbl-wrap.
    ...
    return `<div class="tab-view">
      <div class="card card-pad">
        ...
        ${_hubspot.error ? `<div class="banner" style="margin-top:12px;background:#FFE0E0;color:#8B1A1A;padding:8px 10px;border-radius:6px;font-size:12.5px">Data not loaded — ${escapeHtml(_hubspot.error)}. The GH Action will populate it on its next run.</div>` : ''}
      </div>
